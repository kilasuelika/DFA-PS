...
void TFATradingSystem::set_root(const std::string &dir) { workdir = dir; }
...
void TFATradingSystem::read_config(const std::string &file) {
    ...
    // Update path
    /*config["data_path"] =
        SciLib::path_join(workdir, SciLib::json_string_to_string(config["data_path"].as_string()));
    fmt::print("Setting data_path: \n", config["data_path"].as_string().c_str());
    config["decision_model"].as_object()["precomputed_hidden_factor_path"] = SciLib::path_join(
        workdir,
        SciLib::json_string_to_string(
            config["decision_model"].as_object()["precomputed_hidden_factor_path"].as_string()));
    fmt::print(
        "Setting precomputed_hidden_factor_path: \n",
        config["decision_model"].as_object()["precomputed_hidden_factor_path"].as_string().c_str());*/
};
...
void TFATradingSystem::load(const std::string &name) {
    ...
    // Load model.
    fmt::print("Load FA model.\n");
    if (config["decision_model"].as_object()["use_precomputed_hidden_factor"].as_bool()) {
        std::string name = config["decision_model"]
                               .as_object()["precomputed_hidden_factor_path"]
                               .as_string()
                               .c_str();
        fmt::print("Using precomputed hidden factor. Load FA model from "
                   "precomputed_hidden_factor_path ({})\n",
                   name);
        fa->load(name);
    } else {
